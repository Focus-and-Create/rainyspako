...
const MatchGame = {
    ...
    _restoreProgressState: function() {
        ...
        try {
            ...
            const knownEs = new Set([
                ...this._curriculum.map(w => w.es),
                ...this._pool.map(w => w.es),
                ...this._recentPool.map(w => w.es)
            ]);
            const restoredMastery = {};
            const savedMastery = saved.mastery || {};
            Object.keys(savedMastery).forEach(es => {
                const mastery = Number(savedMastery[es]);
                if (knownEs.has(es) && Number.isFinite(mastery) && mastery > 0) {
                    restoredMastery[es] = Math.floor(mastery);
                }
            });
    ...
    _refill: function() {
        ...
        const finishRefill = () => {
            if (this._selected === null && selectedAtStart !== null && this._cards[selectedAtStart] && !this._cards[selectedAtStart].matched) {
